...
select department, name, salary from employee order by salary DESC;
...
select * from ( select salary, rank() over (order by salary DESC) as SalaryRank from employee) where SalaryRank = 4;
